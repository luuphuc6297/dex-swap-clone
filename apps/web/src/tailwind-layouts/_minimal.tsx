...
import { MenuItems } from 'tailwind-layouts/sidebar/_layout-menu';

function filterLanguages(languages: Array<{ locale: string; language: string; code: string }>) {
    const filteredLanguages = filter(languages, (language) => {
        return language.code === 'en' || language.code === 'ko';
    });
...
function HeaderRightArea() {
    ...
    const filtered = filterLanguages(languageList);
            ...
            <div className="hidden items-center gap-6 lg:flex 2xl:gap-8">
                {filtered && (
                    <LangSelector
                        ...
                        // @ts-ignore
                        style={{}}
                        className="origin-top-right translate-y-0 rounded-lg  opacity-100 ltr:right-0 rtl:left-0"
                    />
                )}
