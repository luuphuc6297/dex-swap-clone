...
const AccessRiskComponent: React.FC<AccessRiskProps> = ({ token }) => {
  ...
  const { targetRef, tooltip, tooltipVisible } = useTooltip(
    <>
      <p className='text-left text-sm leading-loose tracking-tight text-gray-700 dark:text-gray-400'>{t('Risk scan results are provided by a third party')}</p>
      <Link style={{ display: 'inline' }} ml="4px" external href="https://www.avengerdao.org">
        AvengerDAO
      </Link>
      {tokenInLists || data?.riskLevel > TOKEN_RISK.MEDIUM ? (
        <>
          <p className='text-left text-sm leading-loose tracking-tight text-gray-700 dark:text-gray-400'>
            {t(
          ...
          <Flex mt="4px">
            <p className='text-left text-sm leading-loose tracking-tight text-gray-700 dark:text-gray-400'>{t('Learn more about risk rating')}</p>
