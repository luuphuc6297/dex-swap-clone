...
export const VotingBoxBorder = styled(Box)<{ hasBoosted?: boolean }>`
    ...
    border-style: solid;
    border-color: ${({ theme, hasBoosted }) =>
        hasBoosted ? 'transparent' : theme.colors.cardBorder};
    background: ${({ hasBoosted }) =>
        hasBoosted ? 'linear-gradient(180deg, #53dee9, #7645d9)' : 'transparent'};
`;
...
export const VotingBoxCardInner = styled(Box)<{ hasBoosted?: boolean }>`
    ...
    background: ${({ theme, hasBoosted }) =>
        hasBoosted ? theme.colors.gradientBubblegum : 'transparent'};
`;
