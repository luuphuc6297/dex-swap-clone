    ...
    async function onRemove() {
        ...
        for (let i = 0; i < methodNames.length; i++) {
            ...
            try {
                ...
                const val = await routerContract.estimateGas[methodNames[i]](...args);
                // eslint-disable-next-line no-await-in-loop
                safeGasEstimate = calculateGasMargin(val);
            } catch (e) {
    ...
    const handleSelectCurrencyA = useCallback(
        (currency: Currency) => {
            if (currencyIdB && currencyId(currency) === currencyIdB) {
                router.replace(`/v2/remove/${currencyId(currency)}/${currencyIdA}`, undefined, {
                    shallow: true,
                });
            } else {
                router.replace(`/v2/remove/${currencyId(currency)}/${currencyIdB}`, undefined, {
                    shallow: true,
                });
            }
        },
    ...
    const handleSelectCurrencyB = useCallback(
        (currency: Currency) => {
            if (currencyIdA && currencyId(currency) === currencyIdA) {
                router.replace(`/v2/remove/${currencyIdB}/${currencyId(currency)}`, undefined, {
                    shallow: true,
                });
            } else {
                router.replace(`/v2/remove/${currencyIdA}/${currencyId(currency)}`, undefined, {
                    shallow: true,
                });
            }
        },
...
export const RemoveLiquidityLayout = ({ currencyA, currencyB, children, pair }) => {
    ...
    return (
        <Page>
            <AppBody className="shadow-card dark:bg-light-dark">
                <AppHeader
                    backTo="/liquidity"
