    ...
    const handleSelectCurrencyA = useCallback(
        (currency: Currency) => {
            if (currencyIdB && currencyId(currency) === currencyIdB) {
                router.replace(
                    `/v2/remove/${currencyId(currency)}/${currencyIdA}?stable=1`,
                    undefined,
                    { shallow: true }
                );
            } else {
                router.replace(
                    `/v2/remove/${currencyId(currency)}/${currencyIdB}?stable=1`,
                    undefined,
                    { shallow: true }
                );
            }
    ...
    const handleSelectCurrencyB = useCallback(
        (currency: Currency) => {
            if (currencyIdA && currencyId(currency) === currencyIdA) {
                router.replace(
                    `/v2/remove/${currencyIdB}/${currencyId(currency)}?stable=1`,
                    undefined,
                    { shallow: true }
                );
            } else {
                router.replace(
                    `/v2/remove/${currencyIdA}/${currencyId(currency)}?stable=1`,
                    undefined,
                    { shallow: true }
                );
            }
