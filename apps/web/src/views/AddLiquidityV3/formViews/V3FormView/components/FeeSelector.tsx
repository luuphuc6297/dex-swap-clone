...
export default function FeeSelector({
    ...
    const farmV3 = useMemo(() => {
        if (currencyA && currencyB) {
            ...
            const [token0, token1] = tokenA.sortsBefore(tokenB)
                ...
                : [tokenB, tokenA];
            return (
                farmV3Config?.length > 0 &&
                farmV3Config.find((f) => f.token.equals(token0) && f.quoteToken.equals(token1))
            );
        }
        return null;
    }, [currencyA, currencyB, farmV3Config]);

    const { isLoading, isError, largestUsageFeeTier, distributions, largestUsageFeeTierTvl } =
    ...
    const v2PairHasBetterTokenAmounts = useMemo(() => {
        ...
        if (largestUsageFeeTierTvl) {
            ...
            return (
                pair.reserve0.greaterThan(
                    tryParseAmount(String(largestUsageFeeTierTvl[0]), pair.token0)
                ) ||
                pair.reserve1.greaterThan(
                    tryParseAmount(String(largestUsageFeeTierTvl[1]), pair.token1)
                )
            );
        }
        return true;
    ...
    return (
        <HideShowSelectorSection
            ...
            heading={
                feeAmount ? (
                    <AutoColumn gap="8px">
                        <Text>LP - {FEE_AMOUNT_DETAIL[feeAmount].label}% fee tier</Text>
                        {distributions && (
