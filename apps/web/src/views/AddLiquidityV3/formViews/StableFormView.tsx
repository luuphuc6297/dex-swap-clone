...
import { AutoColumn, AutoRow, Box, Dots, Flex, QuestionHelper, RowBetween, Text } from '@pancakeswap/uikit'
...
export default function StableFormView({
  ...
  if (addIsUnsupported || addIsWarning) {
    buttons = (
      <Button disabled style={{ marginBottom: 32 }}>
        {t('Unsupported Asset')}
      </Button>
    )
  } else if (!account) {
      ...
      <AutoColumn gap="md">
        {shouldShowApprovalGroup && (
          <RowBetween style={{ gap: '8px' }}>
            {showFieldAApproval && (
              <Button onClick={approveACallback} disabled={approvalA === ApprovalState.PENDING} fullWidth>
            ...
            {showFieldBApproval && (
              <Button onClick={approveBCallback} disabled={approvalB === ApprovalState.PENDING} fullWidth>
  ...
  return (
    <>
      ...
      <RightContainer>
        <AutoColumn>
          <Box>
            <Text className="mb-8 text-base font-bold uppercase" bold fontSize="12px" textTransform="uppercase" color="secondary">
              Pool Reserves
            </Text>
