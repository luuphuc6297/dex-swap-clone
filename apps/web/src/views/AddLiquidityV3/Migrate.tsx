...
export function Migrate({ v2PairAddress }: { v2PairAddress: string }) {
    ...
    const { data: token0Address } = useSWRContract([pairContract, 'token0'], {
        use: [immutableMiddleware],
    });
    const { data: token1Address } = useSWRContract([pairContract, 'token1'], {
        use: [immutableMiddleware],
    });
...
function V2PairMigrate({
    ...
    const token0Value = useMemo(
        () =>
            CurrencyAmount.fromRawAmount(
                token0,
                JSBI.divide(
                    JSBI.multiply(JSBI.BigInt(pairBalance.toString()), reserve0.quotient),
    ...
    const token1Value = useMemo(
        () =>
            CurrencyAmount.fromRawAmount(
                token1,
                JSBI.divide(
                    JSBI.multiply(JSBI.BigInt(pairBalance.toString()), reserve1.quotient),
                    v2LPTotalSupply.quotient
                )
            ),
    ...
    const isSuccessfullyMigrated =
        !!pendingMigrationHash && JSBI.equal(JSBI.BigInt(pairBalance.toString()), ZERO);
        ...
        <CardBody>
            <ResponsiveTwoColumns>
                <AutoColumn alignSelf="start" gap="16px">
                    ...
                    />
                    <AutoColumn gap="8px">
                        <p className="mb-8 text-left font-medium tracking-tighter text-gray-900 dark:text-white xl:text-2xl 3xl:mb-8 3xl:text-[32px]">
                            Deposit Amount
                        </p>
                        <GreyCard>
                            <AutoColumn gap="8px">
                                <AutoRow justifyContent="space-between">
                                    ...
                                    {position && (
                                        <Text bold>{position.amount0.toSignificant(6)}</Text>
                                    )}
                                </AutoRow>
                                <AutoRow>
                                    <AutoRow gap="4px" flex={1}>
                                    ...
                                    {position && (
                                        <Text bold>{position.amount1.toSignificant(6)}</Text>
                                    )}
                                </AutoRow>
                ...
                <AutoColumn alignSelf="flex-start" gap="16px">
                    <RowBetween>
                        <p className="mb-2 text-base font-medium uppercase">Set Price Range</p>
                        <RateToggle
                    ...
                    {largePriceDifference ? (
                        <GreyCard>
                            ...
                            <Text fontSize={14} style={{ marginTop: 8, fontWeight: 400 }}>
                                You should only deposit liquidity into PancakeSwap V3 at a price you
                                believe is correct. <br />
                                If the price seems incorrect, you can either make a swap to move the
                                price or wait for someone else to do so.
                            </Text>
                        </GreyCard>
                    ...
                    {showCapitalEfficiencyWarning ? (
                        ...
                        <Button
                            className="relative inline-flex h-10 shrink-0 items-center justify-center overflow-hidden rounded-full border-white bg-white px-5 text-center text-xs font-medium tracking-wider text-gray-900 shadow-card outline-none transition-all hover:-translate-y-0.5 hover:shadow-large focus:-translate-y-0.5 focus:shadow-large focus:outline-none dark:bg-light-dark dark:text-white sm:h-12 sm:px-8 sm:text-sm md:h-10 md:px-5 xl:h-12 xl:px-7"
                            onClick={() => {
