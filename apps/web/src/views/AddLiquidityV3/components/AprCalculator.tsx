...
const deriveUSDPrice = (
    baseUSDPrice?: Price<Currency, Currency>,
    pairPrice?: Price<Currency, Currency>
) => {
...
export function AprCalculator({
    ...
    const currencyAUsdPrice = baseUSDPrice
        ...
        : deriveUSDPrice(
              quoteUSDPrice,
              price?.baseCurrency.equals(quoteCurrency?.wrapped) ? price : price?.invert()
          );
    const currencyBUsdPrice =
        baseUSDPrice &&
        (deriveUSDPrice(
            baseUSDPrice,
            price?.baseCurrency.equals(baseCurrency?.wrapped) ? price : price?.invert()
        ) ||
    ...
    const depositUsd = useMemo(
        () =>
            ...
            currencyBUsdPrice &&
            String(
                parseFloat(amountA.toExact()) * currencyAUsdPrice +
                    parseFloat(amountB.toExact()) * currencyBUsdPrice
            ),
    ...
    const onApply = useCallback(
        (position: RoiCalculatorPositionInfo) => {
            batch(() => {
                const isToken0Price = position.amountA?.wrapped.currency.sortsBefore(
                ...
                if (position.fullRange) {
                    ...
                    onBothRangeInput({
                        leftTypedValue: isToken0Price
                            ? position.priceLower?.toFixed()
                            : position.priceUpper?.invert().toFixed(),
                        rightTypedValue: isToken0Price
                            ? position.priceUpper?.toFixed()
                            : position.priceLower?.invert().toFixed(),
                    });
                }
