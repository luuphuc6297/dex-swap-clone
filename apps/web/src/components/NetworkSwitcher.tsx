...
const NetworkSelect = ({ switchNetwork, chainId }) => {
    ...
    return (
        <div className="w-64 origin-top-right translate-y-0 rounded-lg  p-3 opacity-100 ltr:right-0 rtl:left-0 ">
            <Menu>
                ...
                {chains
                    // .filter(
                    //     (chain) => !('testnet' in chain && chain.testnet) || chain.id === chainId
                    // )
                    .map((chain) => (
                        <UserMenuItem
                            key={chain.id}
