...
describe('utils', () => {
    describe('#getBscScanLink', () => {
        it('correct for tx', () => {
            expect(getBlockExploreLink('abc', 'transaction', ChainId.BSC)).toEqual(
                'https://bscscan.com/tx/abc'
            );
        });
        it('correct for token', () => {
            expect(getBlockExploreLink('abc', 'token', ChainId.BSC)).toEqual(
                'https://bscscan.com/token/abc'
            );
        });
        it('correct for address', () => {
            expect(getBlockExploreLink('abc', 'address', ChainId.BSC)).toEqual(
    ...
    describe('#calculateSlippageAmount', () => {
        it('bounds are correct', () => {
            const tokenAmount = CurrencyAmount.fromRawAmount(
                new Token(ChainId.BSC, AddressZero, 0),
                '100'
            );
            ...
            expect(
                calculateSlippageAmount(tokenAmount, 10000).map((bound) => bound.toString())
            ).toEqual(['0', '200']);
    ...
    describe('#isAddress', () => {
        ...
        it('returns the checksummed address', () => {
            expect(isAddress('0xC36C71664582073F76C3B88C2D5Ca62EdB5fBa20')).toBe(
                '0xC36C71664582073F76C3B88C2D5Ca62EdB5fBa20'
            );
            expect(isAddress('0xC36C71664582073F76C3B88C2D5Ca62EdB5fBa20')).toBe(
                '0xC36C71664582073F76C3B88C2D5Ca62EdB5fBa20'
            );
