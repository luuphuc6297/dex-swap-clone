...
import { modalWrapperClass, promotedGradientClass, walletIconClass } from './WalletModal.css'
...
function MobileModal<T>({
  ...
  return (
    <div className=" shadow-large mt-5 origin-top-right translate-y-0 rounded-lg bg-white p-3 opacity-100 ltr:right-0 rtl:left-0 dark:bg-gray-800">
      {error ? (
        <AtomBox
          display="flex"
...
function WalletSelect<T>({
  ...
  const filteredWallet = wallets.filter((wallet) => wallet.id === 'metamask')
  const walletDisplayCount = wallets.length > displayCount ? displayCount - 1 : displayCount
  const walletsToShow = showMore ? filteredWallet : filteredWallet.slice(0, walletDisplayCount)
...
function DesktopModal<T>({
  ...
  return (
    <div className="xs:w-full relative z-50 inline-block w-full scale-100 text-left align-middle opacity-100">
      <div className="dark:bg-light-dark relative z-50 mx-auto w-[440px] max-w-full rounded-lg bg-white px-9 py-16">
        {/* <AtomBox
          display="flex"
          flexDirection="column"
          bg="backgroundAlt"
        ...
        > */}
        {/* <AtomBox px="48px"> */}
        <p className="mb-4 text-center text-2xl font-medium uppercase text-gray-900 dark:text-white">
          {t('Connect Wallet')}
        </p>
        <p className="text-center text-sm leading-loose tracking-tight text-gray-600 dark:text-gray-400">
          {t(
