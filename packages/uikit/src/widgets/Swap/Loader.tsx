...
export default function Loader({
  ...
  return (
    <Component className={cn("flex items-center gap-2", variant === "moveUp" && handleLoaderPosition(size), className)}>
      <span className={cn("rounded-full bg-current", handleVariantClasses(variant, size), sizes[size])} />
      <span
        className={cn("animation-delay-200 rounded-full bg-current", handleVariantClasses(variant, size), sizes[size])}
      />
      <span
        className={cn("animation-delay-500 rounded-full bg-current", handleVariantClasses(variant, size), sizes[size])}
      />
